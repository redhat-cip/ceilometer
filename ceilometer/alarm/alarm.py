...
class Alarm(models.Model):
    ...
    def match(self, meter):
        ...
        For now only project_id and user_id metadata are handled
        """

        project_id = self.matching_metadata.get('project_id', None)
        user_id = self.matching_metadata.get('user_id', None)

        return self.counter_name == meter['counter_name'] \
            and (not project_id or project_id == meter['project_id']) \
            and (not user_id or user_id == meter['user_id'])
