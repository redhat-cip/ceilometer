...
class AlarmService(service.PeriodicService):
    ...
    def _cache_aggregated_metric_list(self, alarm_id):
        ...
        return sorted(self._cache_aggregated_metric[alarm_id].values(),
                      key=lambda a: a.timestamp, reverse=True)

    def _cache_aggregated_metric_delete(self, alarm, aggregates):
        for agg in aggregates:
            self.storage_conn.aggregated_metric_delete(agg.id)
            del self._cache_aggregated_metric[alarm.id][agg.id]
